...
namespace TrainingCenter.LinqToObjectsDemo.Titanic
{
	class Program
	{
		static void Main(string[] args)
		{
            ...
            // Distinct
            string[] allFamilyNames = passengers.Select(pax => pax.FamilyName).Distinct().OrderBy(name => name).ToArray();
            // Все возможные цены на билеты. Используем тот факт что Price реализует интерфейс IEquatable<T>
            Price[] allPrices = passengers.Select(pax => pax.TicketPrice)
                .Distinct()
                .OrderBy(price => price)
                .ToArray();
