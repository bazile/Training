...
namespace EnvironmentInformation
{
    ...
    {
        const string EnvGroup             = "System.Environment";
        const string BitConvGroup         = "System.BitConverter";
        const string PathGroup            = "System.IO.Path";
        const string SysInfoGroup         = "System.Windows.Forms.SystemInformation";
        ...
        public MainForm()
        {
            ...
            _initTabAction = new Action[] { InitSystemInfoData, InitVarsData, InitPathsData };
        }

        private void OnLoad(object sender, EventArgs e)
        {
            InitTab(tabControl.SelectedIndex);
        ...
        void InitPathsData()
        {
            ...
            Environment.SpecialFolder[] folders = (Environment.SpecialFolder[])Enum.GetValues(typeof(Environment.SpecialFolder));
            Array.Sort(folders, (f1, f2) => f1.ToString().CompareTo(f2.ToString()));

            foreach (var folder in folders)
            {
                ListViewItem listItem = new ListViewItem(folder.ToString());

                string folderPath = "";
                try { folderPath = Environment.GetFolderPath(folder); }
                catch { }

                listItem.SubItems.Add(folderPath);
                listViewPaths.Items.Add(listItem);
            }
        ...
        {
            if (_initTabAction[tabIndex] != null)
            {
                _initTabAction[tabIndex]();
                _initTabAction[tabIndex] = null;
            }
        }
